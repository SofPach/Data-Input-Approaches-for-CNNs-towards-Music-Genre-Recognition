        ...
        def __next__(self):
            ...
            for tid in tids:
                try:
                    ...
                    self.Y[batch_size] = Y.loc[tid].values
                    batch_size += 1
                    print(tid)
            ...
            with self.lock2:
                ...
                #return torch.from_numpy(np.array(self.X[:int(batch_size)],dtype='float32')).reshape(1, *self.X[:int(batch_size)].shape).permute(1,0,2,3), torch.from_numpy(np.array(self.Y[:int(batch_size)],dtype='float32'))
